...
get_jags_model <- function(base_filename = "jags", model){
  ...
  if(model == "BR"){
   ...
   #add array for which node has data, replace 1:obs_nbnode_n with array name
    ...
    for (i in 1:obs_brnode_n){
        dta_conn_full.exp[i,1:obs_nbnode_n] <- conn[i,]/sum(conn[i,]) * dta_conn_full_rowsum[i] # expected value
        for (j in 1:obs_nbnode_n){
            dta_conn_full.rep[i,j] ~ dbinom(conn[i,j]/sum(conn[i,]), dta_conn_full_rowsum[i])

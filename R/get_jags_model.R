...
get_jags_model <- function(base_filename = "jags", model){
  ...
  if(model == 1){
    ...
    ############################################################################
    # Connectivity Model -
    # using assignment data as a breeding x winter matrix, with the inferred nodes as rows (e.g. genetic assignment)
    # connectivity data in :
    # obs_bnode_n = number of breeding nodes
    # obs_wnode_n = number of nonbreeding nodes
    # wnode_gendat = indices for nonbreeding node in connectivity matrix with data
    # bnode_gendat  = indices for breeding node in connectivity matrix with data
    ...
    ############################################################################
    # Connectivity Model -
    # using assignment data as a winter x breeding matrix, with the inferred nodes as rows (e.g. geolocator assignment of breeding individuals to nonbreeding populations)
    # obs_bnode_n = number of breeding nodes
    # obs_wnode_n = number of nonbreeding nodes
    # wnode_gldat = indices for nonbreeding node in connectivity matrix with data
    # bnode_gldat  = indices for breeding node in connectivity matrix with data
    ...
    # dta_conn_gleffort = relative trapping effort at each winter node, i.e. colSums(dta_conn_glx)/sum(dta_conn_glx)
    # dta_conn_nb.est.mn = mean population indices for breeding nodes, must be integers
    # dta_conn_nb.est.sum = sum of dta_conn_nb.est.mn
    # dta_conn_nw.est.mn = mean population indices for nonbreeding nodes,  must be integers
    # dta_conn_nw.est.sum = sum of dta_conn_nw.est.mn

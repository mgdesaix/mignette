...
The sooner we get a decided upon name, the less work it will be for me in the future to switch this project over!
...
ne_land <- ne_countries(scale = ne_scale, returnclass = "sf") %>%
  ...
  st_union() %>% 
  st_geometry()
ne_land_proj <- st_transform(ne_land, crs = mollweide)
```
...
Now we have all the input data we need to get the polygons of the range. Our function `get_range_smooth()` takes care of this. **NOTE:** Make sure to check the layer names in your input projected seasonal abundance raster, they should follow the convention "nonbreeding", "prebreeding_migration", "breeding", and "postbreeding_migration". If they do not, make sure to rename them accordingly!

```
# this fuction can take a while (~15 min.)
range_smooth <- get_range_smooth(abd_season_proj = abd_season_proj, 
                                  species = "comyel", 
...
Extracting a single polygon of a portion of the range is simple and quick! Here's an example of getting the **breeding** season range from the `get_range_smooth()` output.
...
breed_season <- dplyr::filter(range_smooth,
  ...
  out.name.breed.wgs <- paste0(species, ".", season, ".sf.smooth.WGS84.ebird")
